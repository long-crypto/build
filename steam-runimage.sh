...
run_install() {
	...
	INSTALL_PKGS=(
		steam egl-wayland Mesa-libVulkan-radeon Mesa-libVulkan-radeon-32bit
		Mesa-libVulkan-intel Mesa-libVulkan-intel-32bit Mesa-libVulkan-nouveau
		libpipewire-0_3-0 libpipewire-0_3-0-32bit pipewire
		libpulse0 libpulse0-32bit libvkd3d1 libvkd3d1-32bit wget
		vulkan-layers vulkan-layers-32bit libfreetype6 libfreetype6-32bit libfuse2
		mangohud mangohud-32bit gamescope gamemode zenity steam-screensaver-fix
	)
	...
	zypper --non-interactive install glibc-eac glibc-eac-32bit
	...
	zypper --non-interactive remove --clean-deps llvm-libs || true
	# same for glycin
	zypper --non-interactive remove --clean-deps glycin || true

	VERSION=$(rpm -q steam --queryformat '%{VERSION}' 2>/dev/null)
	[ -n "$VERSION" ] && echo "$VERSION" > ~/version

	echo '== shrink (optionally)'
	zypper --non-interactive remove --clean-deps wget gocryptfs jq gnupg perl
	rim-shrink --all
	zypper --non-interactive remove --clean-deps binutils gettext


	rpm -qia | awk -F': ' '/Name/ {name=$2}
	/Size/ {size=$2}
...
rm -rfv ./AppDir/sharun/bin/chisel \
	...
	./AppDir/rootfs/usr/bin/rsvg-convert \
	./AppDir/rootfs/usr/*/*zypper* \
	./AppDir/rootfs/usr/share/gir-1.0 \
	./AppDir/rootfs/var/cache/zypp \
	./AppDir/rootfs/etc/zypp* \
